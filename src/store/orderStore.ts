...
import type { Order, OrderStatus, OrderPriority, OrderFilters, KitchenStats, OrderStore } from '../types';
  ...
  devtools(
    (set, get) => ({
      ...
      markOrderReady: (orderId) => {
        ...
        const acceptedAt = order.acceptedAt ? new Date(order.acceptedAt) : new Date();
        const actualTime = Math.round((new Date(completedAt).getTime() - acceptedAt.getTime()) / (1000 * 60));
