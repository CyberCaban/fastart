...
import type { Order, KitchenStats } from '../types';
...
class SocketService {
  ...
  private setupEventListeners(): void {
    ...
    this.socket.on('order:new', (order: Order) => {
      ...
      this.showBrowserNotification(`Новый заказ #${order.orderNumber}`, `Стол ${order.tableNumber}`);
    });
  ...
  private handleReconnect(): void {
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      ...
      useOrderStore.getState().setError('Не удалось подключиться к серверу. Проверьте соединение.');
      return;
    }
